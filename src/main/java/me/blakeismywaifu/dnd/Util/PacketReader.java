...
import me.blakeismywaifu.dnd.Events.RightClickNPC;
...
public class PacketReader {
	...
	public void inject(Player player) {
		...
		channel.pipeline().addAfter("decoder", "PacketInjector", new MessageToMessageDecoder<PacketPlayInUseEntity>() {
			...
			@Override
			protected void decode(ChannelHandlerContext channelHandlerContext, PacketPlayInUseEntity packet, List<Object> list) {
				list.add(packet);
		...
		if (packet.getClass().getSimpleName().equalsIgnoreCase("PacketPlayInUseEntity")) {
			...
			if (getValue(packet, "action").toString().equalsIgnoreCase("INTERACT")) {
				for (EntityPlayer npc : NPC.getNPCs()) {
					if (npc.getId() != (int) getValue(packet, "a")) return;
